...
const Signup = () => {
  const [formData, setFormData] = useState({
    lastname: '', username: '', email: '', phone: '', age: '', password: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('http://localhost:3000/auth/register-patient', formData);
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-blue-100 rounded-full mb-4">
            <FaUserAlt className="w-8 h-8 text-blue-600" />
          </div>
          <h1 className="text-2xl font-bold text-gray-800">Inscription Patient</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            S inscrire en tant que Docteur
          </button>
